...
class MVX_Ajax {
    ...
    public function __construct() {
        ...
        add_action('wp_ajax_mvx_suspend_vendor', array(&$this, 'mvx_suspend_vendor'));
        add_action('wp_ajax_mvx_activate_vendor', array(&$this, 'mvx_activate_vendor'));
        ...
        // vendor management tab under MVX
        add_action('wp_ajax_mvx_get_vendor_details', array(&$this, 'mvx_get_vendor_details'));
    ...
    function single_product_multiple_vendors_sorting() {
        ...
        $MVX->template->get_template('single-product/multiple_vendors_products_body.php', array('more_product_array' => $more_product_array, 'sorting' => $sorting_value));
        die;
    }
    ...
    function mvx_auto_suggesion_product() {
        ...
        if (count($product_objects) > 0) {
            ...
            $html .= "<ul><li class='mvx_no-suggesion'>" . __('No Suggestion found', 'dc-woocommerce-multi-vendor') . "</li></ul>";
        }
    ...
    public function mvx_dismiss_dashboard_message() {
        ...
        if ($is_updated) {
            ...
            if (is_array($msgs_array) && !empty($msgs_array) && count($msgs_array) > 0) {
                ...
                ?> </span><br/>
                <a href="<?php echo get_permalink(get_option('mvx_product_vendor_messages_page_id')); ?>"><button><?php echo __('DETAILS', 'dc-woocommerce-multi-vendor'); ?></button></a>
                <div class="clear"></div>
    ...
    function search_product_data() {
        ...
        if ($product_id) {
            ...
            //$product_orders = get_mvx_vendor_orders(array('product_id' => $product_id));
    ...
    public function banking_overview_search() {
        ...
        if ( $vendor ) {
            ...
            $vendor_all_ledgers = apply_filters('mvx_admin_report_banking_data', $data_store->get_ledger( array( 'vendor_id' => $vendor->id ), '', $requestData )); 
    ...
    function vendor_report_sort() {
        ...
        if( $total_sales_data ){
            ...
            foreach ($total_sales_data as $vendor_id => $report) {
                $vendor = get_mvx_vendor( $report['vendor_id'] );
                if( $vendor ){
    ...
    function dismiss_vendor_to_do_list() {
        ...
        if ($type == 'user') {
            ...
            update_post_meta($id, '_dismiss_to_do_list', 'true');
            $comment_id = MVX_Product::add_product_note($id, $reason, get_current_user_id());
            ...
            update_post_meta($id, '_dismiss_to_do_list', 'true');
            wp_update_post(array('ID' => $id, 'post_status' => 'mvx_canceled'));
        }
        die();
    }
    ...
    function activate_pending_vendor() {
        ...
        if ($user_id) {
            ...
            if (isset($custom_note) && $custom_note != '') {
                $mvx_vendor_rejection_notes = unserialize(get_user_meta($user_id, 'mvx_vendor_rejection_notes', true));
                $mvx_vendor_rejection_notes[time()] = array(
                    'note_by' => $note_by,
                    'note' => $custom_note);
                update_user_meta($user_id, 'mvx_vendor_rejection_notes', serialize($mvx_vendor_rejection_notes));
            }
        }
                            ...
                            <div class="modal-dialog">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" data-comment_id="' . $comment->comment_ID . '" data-vendor_id="' . get_current_vendor_id() . '" class="btn btn-default mvx-comment-reply">' . __('Comment', 'dc-woocommerce-multi-vendor') . '</button>
                                    </div>
                                </div>
                        ...
                        $row .='<div class="modal fade" id="qna-reply-modal-' . $data->ques_ID . '" role="dialog">
                                    <div class="modal-dialog">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                <button type="button" data-key="' . $data->ques_ID . '" class="btn btn-default mvx-add-qna-reply">' . __('Add', 'dc-woocommerce-multi-vendor') . '</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                ...
                if ($product) {
                    ...
                    if (!isset($have_answer[0])) {
                        ...
                        $details .= '<div class="mvx-question-details-modal modal-body">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" data-key="' . $question->ques_ID . '" class="btn btn-default mvx-add-qna-reply">' . __('Add', 'dc-woocommerce-multi-vendor') . '</button>
                                    </div>';
                        ...
                        if (apply_filters('mvx_vendor_can_modify_qna_answer', false)) {
                            $details .= '<div class="mvx-question-details-modal modal-body">
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" data-key="' . $have_answer[0]->ans_ID . '" class="btn btn-default mvx-update-qna-answer">' . __('Edit', 'dc-woocommerce-multi-vendor') . '</button>
                                    </div>';
                        } else {
                            $details .= '<div class="mvx-question-details-modal modal-body">
    ...
    function mvx_get_vendor_details() {
        global $MVX;
        ...
        if (isset($_GET['vendor_id']) && isset($_GET['vendor_id']) && isset($_GET['nonce'])) {
            ...
            if (!wp_verify_nonce($nonce, 'mvx-vendors'))
            ...
            if ($vendor) {
                ...
                if (isset($vendor_report_data[30]) && is_array($vendor_report_data[30])) {
                    $user_info['last_30_days_earning'] = $vendor_report_data[30]['_mvx_stats_table']['current_earning'];
                    $user_info['last_30_days_sales_total'] = $vendor_report_data[30]['_mvx_stats_table']['current_sales_total'];
                    $user_info['last_30_days_withdrawal'] = $vendor_report_data[30]['_mvx_stats_table']['current_withdrawal'];
                    $user_info['last_30_days_orders_no'] = $vendor_report_data[30]['_mvx_stats_table']['current_orders_no'];
                }

                $unpaid_orders = get_mvx_vendor_order_amount(array('commission_status' => 'unpaid'), $vendor->id);
                ...
                $user_info['gateway_logo'] = isset($MVX->payment_gateway->payment_gateways[$vendor->payment_mode]) ? $MVX->payment_gateway->payment_gateways[$vendor->payment_mode]->gateway_logo() : '';
            ...
            if (in_array('dc_vendor', $user->roles)) {
                ...
                if ($is_block) {
                    ...
                    $actions['activate'] = array(
                        'ID' => $user_info['ID'],
                        'ajax_action' => 'mvx_activate_vendor',
                        'url' => '#',
                    ...
                    $actions['suspend'] = array(
                        'ID' => $user_info['ID'],
                        'ajax_action' => 'mvx_suspend_vendor',
                        'url' => '#',
            ...
            if (in_array('dc_pending_vendor', $user->roles) || in_array('dc_rejected_vendor', $user->roles)) {
                ...
                $vendor_application_data = get_user_meta($user_info['ID'], 'mvx_vendor_fields', true);
                ...
                if (!empty($vendor_application_data) && is_array($vendor_application_data)) {
                    foreach ($vendor_application_data as $key => $value) {
                        if ($value['type'] == 'recaptcha')
                            continue;
                        $vendor_application_data_string .= '<div class="mvx-form-field">';
                ...
                $mvx_vendor_rejection_notes = unserialize(get_user_meta($user_info['ID'], 'mvx_vendor_rejection_notes', true));

                $mvx_vendor_custom_notes_html = '';
                if ($mvx_vendor_rejection_notes) :
                    foreach ($mvx_vendor_rejection_notes as $time => $notes) {
                        $author_info = get_userdata($notes['note_by']);
                        $mvx_vendor_custom_notes_html .= '<div class="note-clm"><p class="note-description">' . $notes['note'] . '</p><p class="note_time note-meta">On ' . date("Y-m-d", $time) . '</p><p class="note_owner note-meta">By ' . $author_info->display_name . '</p></div>';
                    }
                endif;

                $user_info['vendor_custom_notes'] = $mvx_vendor_custom_notes_html;
            }
                    ...
                    try {
                        ...
                        if ($vendor->is_shipping_enable()) {
                            ...
                            if (!in_array($vendor->id, $is_shipped) && !$vendor_order_shipped ) {
                                $action_html .= '<a href="javascript:void(0)" title="' . __('Mark as shipped', 'dc-woocommerce-multi-vendor') . '" onclick="mvxMarkeAsShip(this,' . $pending_order->get_id() . ')"><i class="mvx-font ico-shippingnew-icon action-icon"></i></a> ';
                            } else {
                                $action_html .= '<i title="' . __('Shipped', 'dc-woocommerce-multi-vendor') . '" class="mvx-font ico-shipping-icon"></i> ';
                            }
                        }
                        // shipping amount
    ...
    public function admin_get_vendor_shipping_methods_by_zone(){
        ...
        <table class="form-table mvx-shipping-zone-settings wc-shipping-zone-settings">
            <tbody>
                ...
                <?php if ($show_post_code_list) { ?>
                    <tr valign="top" class="hide_if_zone_not_limited">
                        ...
                        <td class="forminp">
                            <input id="select_zone_postcodes" class="form-control" type="text" name="<?php echo 'mvx_shipping_zone[' . $zone_id . '][_select_zone_postcodes]'; ?>" value="<?php echo $postcodes; ?>" placeholder="<?php _e('Postcodes need to be comma separated', 'dc-woocommerce-multi-vendor'); ?>">
                        </td>
                    </tr>
                <?php } ?>
                <tr valign="top" class="">
            ...
            <script type="text/template" id="tmpl-mvx-modal-add-shipping-method">
                ...
                <footer>
                <div class="inner">
                <button id="btn-ok" data-vendor_id="<?php echo $vendor_id; ?>" class="button button-primary button-large mvx-shipping-zone-add-method" data-zone_id="<?php echo $zone_id; ?>"><?php esc_html_e('Add shipping method', 'dc-woocommerce-multi-vendor'); ?></button>
                </div>
                </footer>
                </section>
                </div>
            ...
            <script type="text/template" id="tmpl-mvx-modal-update-shipping-method">
                ...
                <footer>
                <div class="inner">
                <button id="btn-ok" class="button button-primary button-large mvx-shipping-zone-add-method" data-vendor_id="<?php echo $vendor_id; ?>" data-zone_id="<?php echo $zone_id; ?>"><?php esc_html_e('Save changes', 'dc-woocommerce-multi-vendor'); ?></button>
                </div>
                </footer>
                </section>
                </div>
                    ...
                    if ($product_object) {
                        ...
                        if (is_user_mvx_vendor($user_id) && mvx_is_product_type_avaliable($product_object->get_type())) {
                            ...
                            $html .= '<div class="search-result-clm">'
                                    ...
                                    . '<p>' . $product_cats . '</p>'
                                    . '</div>'
                                    . '<a href="javascript:void(0)" data-product_id="' . $product_object->get_id() . '" class="mvx-create-pro-duplicate-btn btn btn-default item-sell">' . __('Sell yours', 'dc-woocommerce-multi-vendor') . '</a>'
                                    . '</div>';
                        } else {
