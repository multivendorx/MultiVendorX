    ...
    public function product_archive_vendor_info() {
        ...
        if (mvx_is_store_page()) {
            ...
            if( $vendor ){
                ...
                $MVX->template->get_template('archive-vendor-info.php', array('vendor_id' => $vendor->id, 'banner' => $vendor->get_image('banner'), 'profile' => $image, 'description' => apply_filters('the_content', $description), 'mobile' => $vendor->phone, 'location' => $address, 'email' => $vendor->user_data->_vendor_display_email));
            }
        }
    }
